...
export function calculateInvoiceItem(
  ...
  if (isInterState) {
    igstAmount = roundMoney(taxableAmount * (GST_CONFIG.IGST_RATE / 100));
  } else {
    cgstAmount = roundMoney(taxableAmount * (GST_CONFIG.CGST_RATE / 100));
    sgstAmount = roundMoney(taxableAmount * (GST_CONFIG.SGST_RATE / 100));
  }
...
export function calculateInvoiceSummary(
  ...
  items.forEach((itemInput) => {
    const result = calculateInvoiceItem(itemInput, isInterState);
